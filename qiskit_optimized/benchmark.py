"""
Benchmarks the optimized classifiers (qc_core.py) against the *original*
qc1()/qc2() + AerSimulator + transpile() approach from ../Parallel_QML.ipynb,
on the same data and parameters, and confirms the costs match exactly.

Method 1 is benchmarked head-to-head on the real 128-point training set.

Method 2's original approach is only benchmarked on small subsets (4, 8, 16
points) -- extrapolating from the measured growth (~600 transpiled gates at
n1=2 controls to ~14,000 at n1=4) the real case (n1=7, 128 points) would take
a prohibitively long time to even transpile, which is itself the point: the
optimized version handles the full 128-point case in ~0.03s/iteration
regardless.
...
# --------------------------------------------------------------------------
# Original (unoptimized) Method 1, copied verbatim from Parallel_QML.ipynb
# --------------------------------------------------------------------------
...
def original_cost_function_1(all_data, labels, variational):
    backend = AerSimulator()  # fresh backend every call, as in the original
    ...
    for i, lb in enumerate(labels):
        ...
        reduced = partial_trace(sv, [1])  # trace out qubit 1, keep qubit 0
        p = reduced.probabilities_dict()
        losses.append(-np.log(p.get(str(int(lb)), 1e-12)))
...
def original_qc2_subset(all_data, variational, n1):
    from qiskit.circuit.library import XGate  # noqa: F401 (kept for parity with original)
    n2 = NUM_FEATURES
...
def load_dataset(family="checkerboard", num=1):
...
def benchmark_method1():
    ...
    t_optimized_cold = time.time() - t0  # includes one-time matrix caching
    t0 = time.time()
    optimized_cost2 = clf1.cost(train_data, train_labels, variational)
    t_optimized_warm = time.time() - t0  # cache already warm
    ...
    print(f"  speedup (warm cache, the relevant number for a 200-iteration training run): "
...
def benchmark_method2_scaling():
    ...
    for subset_size, n1 in [(4, 2), (8, 3), (16, 4)]:
    ...
    print(f"  -> at this rate, a full 200-SPSA-iteration training run (3 cost evals/iteration) "
          f"takes roughly {200 * 3 * t_warm:.1f}s\n")
