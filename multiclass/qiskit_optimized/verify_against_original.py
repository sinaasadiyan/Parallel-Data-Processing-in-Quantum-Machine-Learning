...
multiclass notebook
(../results/checkerboard-4L-4Q-dataset<n>/results_spsa_method{1,2}_*.json)
and re-evaluate them with the optimized classifiers in qc_core.py. Confirms
bit-identical predictions and an exact cost match.
"""
import glob
import json
import os
...
def check_method1(num):
    ...
    print(f"[Method1] dataset{num}: optimized test_acc={test_acc:.4f} "
          f"(original {saved['final_results']['final_test_accuracy']:.4f}), "
          f"prediction agreement={agreement:.4f}")
    return agreement


def check_method2_cost(num):
    train_data, _, _, _ = load_dataset(num)
    ...
    print(f"[Method2] dataset{num}: optimized cost={cost:.6f}  "
